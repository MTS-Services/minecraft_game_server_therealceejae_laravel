...
class VoteVerifier
{
    ...
    public function verifyByPingback(Closure $callback): self
    {
        ...
        $this->verificationMethod = function (User $user, Site $site, array $ips) {
            $cacheKey = "server-listing.vote.sites.{$this->siteDomain}.";
    ...
    public function executePingbackCallback(Request $request)
    {
        ...
        Cache::put("server-listing.vote.sites.{$this->siteDomain}." . $result, true, now()->addMinutes(5));
    ...
    {
        if (!setting('server-listing.vote.ipv4-v6-compatibility', true)) {
            return null;
        }
    ...
    {
        return Site::where('url', 'LIKE', '%' . $this->siteDomain . '/%')
            ->pluck('url')
            ->map(fn($url) => removeHttpFromUrl($url))
            ->toArray();
    }
}

...
function displayVoteAlert(message, level) {
    const statusMessage = document.getElementById('status-message');
    if (statusMessage) {
        statusMessage.innerHTML = '<div class="alert alert-' + level + '" role="alert">' + message + '</div>';
    }
}
...
function initVote() {
    document.querySelectorAll('[data-vote-url]').forEach(function (el) {
        ...
        if (window.username && url.includes('{player}')) {
...
function setupVoteTimers(name) {
    ...
    axios.get(voteNameForm.action.replace('/', '') + '/' + name)
...
function refreshVote(url) {
    setTimeout(function () {
        axios.post(url.replace('/vote', '/done'), {

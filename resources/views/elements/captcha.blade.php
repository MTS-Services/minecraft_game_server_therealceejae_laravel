...
@if (setting('captcha.type') === 'recaptcha')
    ...
    <div class="g-recaptcha" data-sitekey="{{ setting('captcha.site_key') }}" data-callback="submitCaptchaForm"
...
@elseif(setting('captcha.type') === 'hcaptcha')
    ...
    <div class="h-captcha mb-2 @if ($center ?? false) text-center @endif"
        data-sitekey="{{ setting('captcha.site_key') }}" data-theme="{{ $dark ?? false ? 'dark' : 'light' }}"></div>
@elseif(setting('captcha.type') === 'turnstile')
    @push('scripts')
        <script src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer></script>
    @endpush

    <div class="cf-turnstile mb-2 @if ($center ?? false) text-center @endif"
        data-sitekey="{{ setting('captcha.site_key') }}" data-theme="{{ $dark ?? false ? 'dark' : 'light' }}"></div>
@endif

                    ...
                    <tbody>
                        ...
                        @foreach ($plugins as $path => $plugin)
                            <tr>
                                ...
                                <td>
                                    <form method="POST"
                                        action="{{ route('admin.plugins.' . (plugins()->isEnabled($path) ? 'disable' : 'enable'), $path) }}"
                                        class="d-inline-block">
                                        @csrf

                                        <button type="submit" class="btn btn-primary btn-sm">
                                            <i class="bi bi-{{ plugins()->isEnabled($path) ? 'x-lg' : 'check-lg' }}"></i>
                                            {{ trans('messages.actions.' . (plugins()->isEnabled($path) ? 'disable' : 'enable')) }}
                                        </button>
                                    </form>
                                    @if (!plugins()->isEnabled($path) && $plugin->id !== 'server-listing')
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($plugin['premium'] && !$plugin['purchased'])
                                            <a href="{{ $plugin['info_url'] }}" class="btn btn-info btn-sm" target="_blank"
                                                rel="noopener noreferrer">
                                                <i class="bi bi-card"></i>
                                                {{ trans('admin.extensions.buy', ['price' => $plugin['price']]) }}
                                            </a>
                                        @else
                                            <form method="POST"
                                                action="{{ route('admin.plugins.download', $plugin['id']) }}">
                                                @csrf
